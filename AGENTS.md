...
预览与导出（PDF 转曲）必须严格遵守，任何模板/语言都不得违反：
...
- 阿语仍走整段 bidi+整形矢量管线，但**逐字形按上表分字体**（导出 `appendArabicVisualPath`、预览拆 `label-latin`/`composition-percent`）。
...
| bala-prem | `bala-prem` | [.cursor/rules/template-bala-prem.mdc](.cursor/rules/template-bala-prem.mdc) |
...
| 主题 | 规则 |
|------|------|
| PDF 导出转曲管线（中文黑点/阿语 RTL/俄文字距/% 字体） | [.cursor/rules/export-pipeline.mdc](.cursor/rules/export-pipeline.mdc) |
...
- 导出转曲（`src/utils/exportText*.ts`、`svgTextToPaths*.ts` 等）— 按六种块独立处理，见上方专项规则
